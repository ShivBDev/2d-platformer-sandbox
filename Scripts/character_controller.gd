...
@export var DRIFT_ACC				: float = 100.0
...
func _handle_jump_press() -> void:
	if Input.is_action_just_pressed("Jump"):
		_switch_state("jump_squat" if is_on_floor() else "jump")
...
func _apply_ground_friction(gravScale : float = 1.0) -> void:
	velocity.x = move_toward(velocity.x, 0, GROUND_FRICTION * gravScale * _engineDelta)
